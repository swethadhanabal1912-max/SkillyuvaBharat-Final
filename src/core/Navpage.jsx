...
export default function Navpage() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-[1000] font-['Raleway',sans-serif] transition-all duration-300 ${
        scroll
          ? "bg-[rgba(248,244,239,0.98)] backdrop-blur-md border-b border-black/[0.06]"
          : "bg-transparent backdrop-blur-0 border-b border-transparent"
      }`}
    >
      <div className="max-w-[1300px] mx-auto px-6 flex items-center justify-between h-20">
        <Link
          to="/"
        ...
        >
          ...
          <div>
            ...
            <span className="text-[10px] font-bold text-[#999] uppercase mt-0.5 block">
              India's Largest Private Job Fair Network
            </span>
          </div>
        </Link>

        <nav className="hidden min-[1101px]:flex items-center gap-8 ml-auto">
          {links.map((item) => (
            <Link
              key={item.title}
              to={item.href}
              onClick={(e) => handleNavClick(e, item)}
              className={`relative py-2 text-[14.5px] font-bold no-underline transition-all duration-250 ease-out hover:text-[#E8650A] hover:-translate-y-0.5 ${
                active === item.title ? "text-[#111]" : "text-black/50"
              }`}
            >
              {item.title}
              {active === item.title && (
                <motion.div
                  layoutId="underline"
                  className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#E8650A] rounded-[10px]"
                />
              )}
            </Link>
          ))}
        ...
        <div className="hidden min-[1101px]:flex items-center gap-3">
          {/* <Link to="/login" className="inline-flex items-center px-6 py-[10px] rounded-lg border-2 border-[#111] font-extrabold text-[13.5px] bg-transparent no-underline text-[#111] transition-colors hover:bg-[#1A7A2E] hover:border-[#1A7A2E] hover:text-white">Login</Link>
          <Link to="/signup" className="inline-flex items-center px-[26px] py-[11px] rounded-lg bg-[#E8650A] text-white font-extrabold text-[13.5px] border-none no-underline shadow-[0_4px_15px_rgba(0,0,0,0.1)] transition-colors hover:bg-[#1A7A2E]">Sign Up</Link> */}
        </div>

        <button
          className="hidden max-[1100px]:block bg-none border-none cursor-pointer"
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-20 left-0 right-0 bg-white border-b border-[#eee] p-6"
          >
            {links.map((item) => (
              <Link
                key={item.title}
                to={item.href}
                onClick={(e) => handleNavClick(e, item)}
                className="flex justify-between items-center p-[15px] font-extrabold no-underline text-[#111] rounded-lg transition-all duration-200 hover:text-[#E8650A] hover:bg-[#E8650A]/[0.06] hover:pl-5"
              >
                {item.title} <ChevronRight size={18} />
              </Link>
            ))}
